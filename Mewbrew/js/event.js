...
class MenuSystem {
    ...
    setupCart() {
        ...
        // Setup close button
        document.querySelector('.cart-sidebar .btn-outline-secondary')?.addEventListener('click', (e) => {
            e.stopPropagation();
            this.toggleCart();
        });
    }
    ...
    showCheckoutModal() {
        if (this.cart.length === 0) {
            this.showNotification('Your cart is empty! Add items before checkout.', 'warning');
            return;
        }
...
window.addToCart = function(itemName, price) {
    ...
    if (menuData) {
        ...
        // Fallback for items not in menuData
        const fallbackData = {
        ...
        window.menuSystem.addToCart(fallbackData);
    }
};
